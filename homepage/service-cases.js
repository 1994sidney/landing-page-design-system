(() => {
  ...
  const quoteTitle = section.querySelector('.mh-case-quote__meta strong');
  const quoteMeta = section.querySelector('.mh-case-quote__meta span');
  const viewport = section.querySelector('.mh-case-carousel');
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)');

  if (!track || !cards.length || !quoteText || !quoteTitle || !quoteMeta || !viewport) return;
  ...
  const render = (index, restart = true) => {
    ...
    quoteTitle.textContent = card.dataset.quoteTitle || '';
    quoteMeta.textContent = card.dataset.quoteMeta || '';
    moveTrack();
